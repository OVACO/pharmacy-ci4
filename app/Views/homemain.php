        ...
        <div class="card-body p-0">
            ...
            <hr>
                ...
                <button type="button" class="btn btn-success  btn-user btn-block" data-bs-toggle="modal"
                                        ...
                                        <div class="form-group">เรื่องขอรับบริการ:
                                            <small class="form-text text-muted">(ตัวอย่างการเพิ่ม เช่น วัคซีนพาสปอร์ต และขอต่ออายุ/ชำระค่าธรรมเนียม เป็นต้น)</small>
                                            <input class="form-control form-control-user" type="text" required="required" name="content"
                            value="<?php echo isset($row['content'])?$row['content']:''; ?>" />
                    </div>
                ...
                <table class="table table-hover"  id="main-list0">
                    <thead>
                        <tr>
                            <th width="10%">ลำดับ</th>
                            <th width="10%">สถานะการแสดงตัวเลือก</th>
                            <th width="50%">เรื่องขอรับบริการ</th>
                            <th width="10%">แสดง/ไม่แสดง</th>
                            <th width="10%"></th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php foreach($main_title as $row): ?>
                        <tr>
                            <td text-align: center ><?php echo $row['main_id']; ?></td>
                            <td><?php echo $row['status_name']; ?></td>
                            <td><?php echo $row['content']; ?></td>
                            <td>
                            <center>
                            <?php if($row['title_stid'] == '0'){ ?>
                            <a onclick="confirmation(event)" class="btn btn-success btn-sm" href="<?php echo base_url('Main/approve/'.$row['main_id']); ?> ">แสดง</a>
                            <?php }else{ ?>
                            <a onclick="confirmation(event)" class="btn btn-danger btn-sm" href="<?php echo base_url('Main/disapproved/'.$row['main_id']); ?> ">ไม่แสดง</a>
                            <?php } ?>
                            </center>    
                        </td>
                            <td>
                            <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                                ...
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                        <form method="post"
                                                                action="<?php echo base_url('Main/edit'); ?>">
